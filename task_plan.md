...
<!-- WHAT: 这是整个任务的路线图。持久化的工作记忆。 -->
<!-- WHY: 超过 50 次工具调用后，原始目标可能被遗忘。此文件保持目标清晰。 -->

## Goal

基于 BettaFish/MindSpider 已有的 B站/微博爬虫能力，开发独立项目 VoxPop，对网友评论进行**情感标注 → 话题聚合 → 乐观/悲观排行盘点**。

## Current Phase

Phase 2 — 项目骨架已完成，等待 Phase 3 执行
...
- [x] 已创建全部代码文件
- [x] 已上传 GitHub 仓库：qql2/VoxPop

### Phase 3: 本地模型集成 ⏳ pending
...
1. 职业词典覆盖哪些岗位？→ 当前预设 17 个通用类，待定稿
2. 本地模型用哪个？→ 先跑关键词基线，后续可替换 WeiboSentiment
3. 是否复用 InsightEngine 的 WeiboMultilingualSentiment？→ 待评估
4. 是否需要自动调度？→ 先手动，稳定后再考虑定时
...
| 项目定位 | 附属 BettaFish vs **独立项目** | **独立项目** | 只是复用数据库，不耦合代码 |
| 记忆范围 | 全局 MEMORY.md vs **会话内** | **会话内** | 群聊项目记忆不污染全局 |
...
- Never repeat a failed action — mutate approach instead
